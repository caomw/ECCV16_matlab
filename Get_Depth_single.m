...
%mainpath = '/research2/ECCV_journal/with_light/NIR_single_MSE_ang_hinge_4/output';
mainpath = '~/Dropbox/ECCV_result/';
maskpath = '/research2/IR_normal_small/mask';
savepath = '/home/yjyoon/Dropbox/ECCV_result';
% if ~isdir(savepath)
%     mkdir(savepath)
% end
       
        N = im2double(imread(fullfile(mainpath,'preditced_136000.png'))); 
        N = N.*2 -1;
        

        t = 1;
        im = im2double(imread(fullfile(mainpath,'preditced_136000.png')));
        im = im(:,:,1); 

        %figure, imshow(im);

        % load mask image
        im_mask = sprintf('%s/%s/%d/%s',maskpath,folder_set{7},5,M_name);
