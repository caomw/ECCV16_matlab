...
%             end
 ...
 for nn= 1:12 %lights
    fprintf(fileID,'%d, %.6f, %.6f,%.6f,%.6f\n',nn,abs_mean(nn),abs_median(nn),ang_loss(nn),deg_10(nn));
 end
 fprintf(fileID,'all_mean_error,all_median_error,all_mean_angle_error,all_10deg\n');
 fprintf(fileID,'%.6f, %.6f, %.6f,%.6f\n',err_total_mean,err_total_median,aloss_total_mean,A1_total_mean);
 fclose(fileID);
 fclose('all');
